...
console.log("extractJsonObject: THE live failure — ```json fence");
{
  ...
  check(
    "bare ``` fence (no language) → clean JSON",
    same(extractJsonObject("```\n" + obj + "\n```"))
  );
  check(
    "fence with trailing newline/space tolerated",
    same(extractJsonObject("```json\n" + obj + "\n```   "))
  );
}
...
console.log("extractJsonObject: prose / whitespace around the object");
{
  ...
  check(
    "object then trailing prose",
    same(extractJsonObject(obj + "\n\nLet me know if you need more."))
  );
...
{
  check(
    "no braces → returned as-is (judge parse will fail)",
    parses(extractJsonObject("totally not json")) === null
  );
  check("empty string → empty (parse fails)", extractJsonObject("") === "");
  check(
    "non-string → empty",
    extractJsonObject(null) === "" && extractJsonObject(undefined) === ""
  );
}

console.log("");
if (failed) {
  console.log(`${RED}json-extract.test: ${failed} check(s) failed${NC}`);
  process.exit(1);
}
