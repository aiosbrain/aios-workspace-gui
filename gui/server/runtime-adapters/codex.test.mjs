...
test("agent_message completed → delta + assistant_done", () => {
  const out = mapCodexEvent({ type: "item.completed", item: { id: "i1", type: "agent_message", text: "hello world" } });
  assert.deepEqual(out, [{ type: "delta", text: "hello world" }, { type: "assistant_done" }]);
});

test("empty agent_message → nothing", () => {
  assert.deepEqual(mapCodexEvent({ type: "item.completed", item: { id: "i1", type: "agent_message", text: "" } }), []);
});

test("command_execution started → tool_use{shell, command}", () => {
  const out = mapCodexEvent({ type: "item.started", item: { id: "c1", type: "command_execution", command: "ls -la", status: "in_progress" } });
  assert.deepEqual(out, [{ type: "tool_use", name: "shell", input: { command: "ls -la" }, id: "c1" }]);
});

test("command_execution completed → tool_result with output, is_error from status", () => {
  const ok = mapCodexEvent({ type: "item.completed", item: { id: "c1", type: "command_execution", command: "ls", aggregated_output: "file.txt\n", exit_code: 0, status: "completed" } });
  ...
  const bad = mapCodexEvent({ type: "item.completed", item: { id: "c2", type: "command_execution", command: "nope", aggregated_output: "err", status: "failed" } });
  assert.equal(bad[0].is_error, true);
});

test("file_change started → tool_use{apply_patch}; completed → summarized tool_result", () => {
  const started = mapCodexEvent({ type: "item.started", item: { id: "f1", type: "file_change", changes: [{ path: "a.md", kind: "add" }], status: "completed" } });
  assert.equal(started[0].type, "tool_use");
  assert.equal(started[0].name, "apply_patch");
  assert.equal(started[0].id, "f1");

  const done = mapCodexEvent({ type: "item.completed", item: { id: "f1", type: "file_change", changes: [{ path: "a.md", kind: "add" }, { path: "b.md", kind: "update" }], status: "completed" } });
...
test("mcp_tool_call started → tool_use{server/tool}; completed → text from result", () => {
  const started = mapCodexEvent({ type: "item.started", item: { id: "m1", type: "mcp_tool_call", server: "fs", tool: "read", arguments: { p: "x" }, status: "in_progress" } });
  assert.deepEqual(started[0], { type: "tool_use", name: "fs/read", input: { p: "x" }, id: "m1" });

  const done = mapCodexEvent({ type: "item.completed", item: { id: "m1", type: "mcp_tool_call", server: "fs", tool: "read", result: { content: [{ type: "text", text: "data" }] }, status: "completed" } });
  assert.equal(done[0].text, "data");
  assert.equal(done[0].is_error, false);

  const err = mapCodexEvent({ type: "item.completed", item: { id: "m2", type: "mcp_tool_call", server: "fs", tool: "read", error: { message: "boom" }, status: "failed" } });
  assert.equal(err[0].text, "boom");
  assert.equal(err[0].is_error, true);
});

test("web_search → tool_use then tool_result", () => {
  assert.deepEqual(mapCodexEvent({ type: "item.started", item: { id: "w1", type: "web_search", query: "acp spec" } }),
    [{ type: "tool_use", name: "web_search", input: { query: "acp spec" }, id: "w1" }]);
  assert.deepEqual(mapCodexEvent({ type: "item.completed", item: { id: "w1", type: "web_search", query: "acp spec" } }),
    [{ type: "tool_result", id: "w1", text: "searched: acp spec", is_error: false }]);
});

test("turn.failed and fatal error → error event", () => {
  assert.deepEqual(mapCodexEvent({ type: "turn.failed", error: { message: "rate limited" } }), [{ type: "error", message: "rate limited" }]);
  assert.deepEqual(mapCodexEvent({ type: "error", message: "stream died" }), [{ type: "error", message: "stream died" }]);
});

test("error item → error event", () => {
  assert.deepEqual(mapCodexEvent({ type: "item.completed", item: { id: "e1", type: "error", message: "tool blew up" } }), [{ type: "error", message: "tool blew up" }]);
});
